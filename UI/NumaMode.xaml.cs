...
namespace UI
{
    public partial class NumaMode : UserControl
    {
        ...
        public void UpdateAffinity()
        {
            ...
            IEnumerable<IGrouping<int, LogicalProcessorRaw>> groupedProcessors = LogicalCores.GroupBy(p => p.Node);
            foreach (IGrouping<int, LogicalProcessorRaw> group in groupedProcessors)
            {
                GroupBox groupBox = new GroupBox
                {
                    Header = $"Numa Node {group.Key}",
